    ...
    <div className="min-h-screen flex flex-col px-6 py-6 bg-background">
      ...
      <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full animate-fade-in space-y-8">
        ...
        <div className="flex flex-col gap-3">
          {levels.map((level) => (
            <Button key={level} variant={selected === level ? "pill-active" : "pill"} size="full" onClick={() => setSelected(level)}>
              {level}
            </Button>
          ))}
        </div>

        <Button size="full" disabled={!selected} onClick={() => navigate("/recommendations")}>Continue</Button>
      </div>
    </div>
  );
};
