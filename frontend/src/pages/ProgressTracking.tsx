          ...
          <div className="flex justify-center">
            <div className="relative w-36 h-36">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                ...
                <circle cx="60" cy="60" r="50" fill="none" stroke="hsl(var(--primary))" strokeWidth="10" strokeLinecap="round"
                  strokeDasharray={`${2 * Math.PI * 50}`} strokeDashoffset={`${2 * Math.PI * 50 * (1 - pct / 100)}`} className="transition-all duration-700" />
              </svg>
          ...
          <div className="bg-card rounded-lg p-5 shadow-card border border-border space-y-3">
            {lessons.map((lesson, i) => (
              <div key={i} className="flex items-center gap-3">
                {lesson.done ? <CheckCircle2 className="w-5 h-5 text-secondary shrink-0" /> : <Circle className="w-5 h-5 text-muted-foreground shrink-0" />}
                <span className={`font-body text-sm ${lesson.done ? "text-foreground" : "text-muted-foreground"}`}>{lesson.title}</span>
              </div>
            ))}
          </div>

          <Button size="full" onClick={() => navigate("/dashboard")}>Continue</Button>
        </div>
      </div>
    </div>
  );
